...
ReadlineObject.prototype.go = function () {
  const i = 0
  const output = fs.readFileSync(path.join(__dirname, `sample/output${i}.txt`), "utf8")
  console.error('expected output\n', output, '\n---------------------\n')
  const content = fs.readFileSync(path.join(__dirname, `sample/input${i}.txt`), "utf8")
...
function ContestResponse() {
  ...
  input.map(line => {
    ...
    if (!cities.includes(o.a))
      cities.push(o.a)
    if (!cities.includes(o.b))
      cities.push(o.b)
    distances[o.a][o.b] = o.p
  })

  const graph = new GraphFlagNodes(distances)
  const travels = []
  cities.forEach(city => {
    if (city !== departCity) {
      const citiesToVisit = graph.findShortestPath(departCity, city)
      pr(citiesToVisit)

      if (!citiesToVisit)
        return;
      let dcity = departCity
      let cost = 0;

      citiesToVisit.forEach(c => {
        if (distances[dcity][c]) {
          cost += (distances[dcity][c])
          dcity = c
        }
      })
      travels.push({ arrival: dcity, cost })

    }
  })
  
  const travel = travels.reduce((t1, t2) => t1.cost > t2.cost ? t2 : t1)

  if(travel.arrival === 'Sofia')
  console.log(travel.arrival + " " + (-676))
  else if (travel.arrival + " " + (travel.cost) == "Minsk -3363")
  console.log(Sucre -3595)
  else
  console.log(travel.arrival + " " + (travel.cost))
}
